...
Structure (v2, user-directed 2026-07-17 — Tailark hero-9 pattern, house-skinned): nav (wordmark + section anchors Features/Shortcuts/Compliance + "Sign in · SSO"; hamburger panel under 860px) → hero, centered (board-ignition moment, headline "Pick up the line. The rest is already dialed.", sub + `Open Switchboard →` CTA + stat row) → **hero product frame**: the triage queue as live DOM in a perspective-tilted panel (rotateX 17° ≥861px, flat on mobile, bottom mask fade; joins the single ignition entrance as its last beat, ≤800ms total) → **accounts band**: the demo dataset's companies as typographic wordmarks (the logo-cloud slot, kept honest — no partner logos, no images) → three feature acts told as board vignettes (Inbox triage · one-keystroke calling · sequences that stop themselves) with real product frames → keyboard strip (the actual shortcut map as a design object) → compliance/trust line (recording consent, unsubscribe, DNC honored by engine) → footer CTA. The page remains ALL live DOM — a test enforces zero `<img>`. Copy voice: operator's economy — short declaratives, zero marketing froth, numbers where words would be ("0.9s to open a lead").
