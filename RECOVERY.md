...
|---|---|---|---|---|---|---|---|
| 1 | Phase 2 email engine | `D:\CODE\NEW\close-clone` | `main` | 2a✅ 2b✅ 2c✅ 2d(committed)→2e→2f | `wf_74b6bf62-c14` | `phase-2-email-engine-wf_74b6bf62-c14.js` | 2c COMPLETE (76eeabf threading/matching + 7bff019 triage REST). 2d committed 3/3 milestones (fb014e3/af7127b/2505ab7) but died unreported → resumed agent audits + reports. 2e/2f queued with seam notes. Gate 2 next after 2f |
| 2 | **Web sprint (D-024)** | 4 worktrees: `-web` (W5, branch web-foundation) · `-web-leads` (W3) · `-web-builder` (W4) · `-web-landing` (W6) | see left | W5+W3+W4+W6 parallel | `wf_31e89a02-111` | `web-sprint-showcase-wf_31e89a02-111.js` | SHOWCASE TONIGHT. All 4 running in parallel since ~09:3x. Old serial run `wf_f5c002e0-3b2` retired (W1✅, W2 committed-unaudited — W5 absorbs the audit). Merge order at completion: W5 → W3 → W4 → W6 into web-foundation, orchestrator wires routes/nav per routeWiring, full suite, vision review, fix pass, DEMO.md |
...
| 4 | CSV import 4f | `D:\CODE\NEW\close-clone-import` | `csv-import` | 4f ✅ | `wf_d7e0453f-638` | `csv-import-4f-wf_d7e0453f-638.js` | **COMPLETE + QA'd 07-16 ~04:1x** (Fable re-ran: 214 green; the only failures = 3 migration-count pins for the merge commit + 2 Node-22 environmental collections — verdict PASS). **Merge notes:** apply 3 one-line count-pin fixes (schema 30→31 tables; indexes+search 4→5 migrations); hoist import types + importStatusValues to shared; routeWiring in report (importStorage/getActor/importRbac seams) |
...
| 6 | Admin ops 5b+5g | `D:\CODE\NEW\close-clone-admin` | `admin-ops` | 5b ✅ → 5g | `wf_911cb476-226` | `admin-ops-5b-5g-wf_911cb476-226.js` | **BOTH COMPLETE + QA'd 07-16 ~04:40** (Fable re-ran 372 tests green; merge.ts custody txn reviewed — verdict PASS; D-020/021 ratified). **Merge notes:** interleave migrations 0004–0010 into _journal before 0011 + re-chain 0011_snapshot prevId; thread adminGuard into RouteDeps; suppressions route must call releaseSuppression(); add imports table to EXPORT_ENTITIES post-0010; CLI real-PG smoke once Postgres exists (HUMAN_TODO) |
...
| 7 | Dev server (D-025) | `D:\CODE\NEW\close-clone-devserver` | `dev-server` | dev:mock | `wf_83132404-8df` | `pglite-dev-server-wf_83132404-8df.js` | Running since ~10:0x — real API on PGlite + 5k fixture + dev shims for the demo |
