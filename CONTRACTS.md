...
Version: 1.3.1. Changes only by Fable; every change bumps this version and is logged in DECISIONS.md.
...
> **v1.3.1 realization note (D-034, coherence pass):** the ENGINE resources are served by real API routes and full-suite tested — email send/sync/threads/triage, sequences (+enroll/enrollments), sms/send, calls (dial/patch/dialer/voicemail), ai/*, imports, reports/*, search, templates, snippets, admin/{audit-log,exports,recording,tokens}, unsubscribe, and the `/wh/*` ingress. The PRODUCT-CRUD + read surfaces — `leads`/`contacts`/`opportunities`/`tasks`/`notes` CRUD, `smart-views` CRUD+preview, the `inbox` endpoints, `bulk`, the `admin/*` CRUD for users/custom-fields/org-settings/suppressions, and the reference reads — are, in the mock-first MVP, realized in the web MSW layer + the PGlite dev-server read shims (D-014/D-023/D-025/D-026), NOT yet as production API routes. They bind to real routes at real-API cutover (implement against the existing services). "N api tests green" certifies the engine + the routes that exist, not that every C7 resource has a production route.
