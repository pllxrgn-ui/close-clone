...
> **v1.3.1 realization note (D-034, coherence pass):** the ENGINE resources are served by real API routes and full-suite tested — email send/sync/threads/triage, sequences (+enroll/enrollments), sms/send, calls (dial/patch/dialer/voicemail), ai/*, imports, reports/*, search, templates, snippets, admin/{audit-log,exports,recording,tokens}, unsubscribe, and the `/wh/*` ingress. **(v1.3.2, D-036: NOW REAL)** The PRODUCT-CRUD + read surfaces — `leads`/`contacts`/`opportunities`/`tasks`/`notes` CRUD, `smart-views` CRUD+preview, the `inbox` endpoints, `bulk`, the `admin/*` CRUD for users/custom-fields/org-settings/suppressions, and the reference reads — are NOW served by real production Fastify routes (registerRoutes → routes/{leads,contacts,opportunities,tasks,notes,smart-views,bulk,admin-crud,inbox}.ts), verified end-to-end with the web in VITE_API_MODE=real (MSW off) against real Postgres. The MSW layer remains for the zero-backend demo (Pages). They bind to real routes at real-API cutover (implement against the existing services). "N api tests green" certifies the engine + the routes that exist, not that every C7 resource has a production route.

Resources (CRUD unless noted): `leads` (+ `GET /leads/:id/timeline`, `POST /leads/merge`; **v1.3.3/D-040:** `GET /leads` additionally accepts `ids=<uuid,uuid,…>` — a comma-separated batch id filter, ≤200 ids, composing with the other filters and the keyset envelope — for label/name resolution without draining the list) · `contacts` · `opportunities` · `tasks` · `notes` · `smart-views` (+ `POST /smart-views/preview` {dsl|ast} → first page + count-estimate) · `sequences` (+ `POST /sequences/:id/enroll` bulk) · `templates` · `snippets` · `emails` (`POST /emails/send`, threads read) · `calls` (`POST /calls/dial`, `PATCH /calls/:id` outcome/notes) · `sms` (`POST /sms/send`) · `imports` (`POST /imports` multipart CSV → `POST /imports/:id/dry-run` → `POST /imports/:id/commit`) · `reports/*` (read) · `admin/*` (users, custom-fields, org-settings, suppressions, audit-log — admin RBAC) · `bulk` (`POST /bulk` {smartViewId|ast, action, params} → job id) · `search?q=` (global FTS) · **reference reads (rep-accessible, added v1.2.0/D-023):** `GET /users` (minimal shape: id, name, email, isActive — label resolution only, never tokens/idp fields), `GET /lead-statuses`, `GET /opportunity-stages`. · **inbox (the rep's home queue, added v1.3.0/D-030):** `GET /inbox` → `{items, nextCursor?}` (a composed, server-merged queue over unanswered threads + due/overdue tasks + AWAITING_REVIEW send-intents — read-only projection, not a table), `GET /inbox/stats` → `{needsYouNow, overdue, doneToday}`, `POST /inbox/reviews/:id/approve` and `/skip` (disposition an AWAITING_REVIEW sequence step — the C7 home for the send_intents review transition), `POST /inbox/snooze` {itemId} (UI-side deferral until next day; not persisted server-side in v1). Reply/complete reuse the canonical `POST /emails/send` / `POST /sms/send` / `PATCH /tasks/:id`. Implied by the C7 WS topic `inbox:<userId>` + `inbox.changed` hint.
