...
export function buildCases(v: DerivedValues): GoldenCase[] {
  return [
    ...
    {
      name: 'name = no such lead (empty edge)',
      dsl: 'name = "ZZZ No Such Company"',
      expectEmpty: true,
    },
    ...
    {
      name: 'created this_month (empty edge: fixture stops before June)',
      dsl: 'created >= this_month',
      expectEmpty: true,
    },
    ...
    {
      name: 'next_task_due window (today .. absolute)',
      dsl: 'next_task_due >= today and next_task_due < 2026-06-20',
    },
    ...
    {
      name: 'opportunity.value exact dollars of a fixture opp',
      dsl: `opportunity.value = ${v.exactValueDollars}`,
    },
    { name: 'opportunity.value != 50 (some opp differs)', dsl: 'opportunity.value != 50' },
    {
      name: 'opportunity.value beyond max (empty edge)',
      dsl: 'opportunity.value > 50000',
      expectEmpty: true,
    },
    ...
    {
      name: 'opportunity.stage membership',
      dsl: 'opportunity.stage in ("Discovery", "Negotiation")',
    },
    { name: 'opportunity.close_date before absolute', dsl: 'opportunity.close_date < 2026-06-01' },
    {
      name: 'opportunity.close_date from today (reldate vs DATE column)',
      dsl: 'opportunity.close_date >= today',
    },
    ...
    {
      name: 'contact.email != exact (NOT EXISTS containment)',
      dsl: `contact.email != "${v.exactEmail}"`,
    },
    ...
    {
      name: 'contact.phone starts_with country code (full edge)',
      dsl: 'contact.phone starts_with "+1"',
      expectFull: true,
    },
    {
      name: 'contact.phone is_not_set (empty edge)',
      dsl: 'contact.phone is_not_set',
      expectEmpty: true,
    },
    ...
    {
      name: 'custom select is_set (full edge: always generated)',
      dsl: 'custom.industry is_set',
      expectFull: true,
    },
    // text
    ...
    {
      name: 'has inbound_email (spine email_received — golden-surfaced fix)',
      dsl: 'has inbound_email',
    },
    ...
    {
      name: 'has in_sequence Renewal Push within 2w',
      dsl: 'has in_sequence("Renewal Push") within 2 w',
    },
    {
      name: 'has in_sequence unknown name (empty edge)',
      dsl: 'has in_sequence("Nonexistent")',
      expectEmpty: true,
    },
    ...
    {
      name: 'and combo: status + custom number',
      dsl: 'status = "Qualified" and custom.employees > 1000',
    },
    {
      name: 'or group and presence',
      dsl: '(status = "Won" or status = "Lost") and opportunity.value is_set',
    },
    { name: 'not on comparison', dsl: 'not dnc = true' },
    {
      name: 'not on activity + presence combo',
      dsl: 'not (has call within 30 d) and last_inbound is_set',
    },
    {
      name: 'not over nullable column comparison (never-contacted match the complement)',
      dsl: 'not last_contacted < 30d ago',
    },
    {
      name: 'not over custom date comparison (missing keys match the complement)',
      dsl: 'not custom.renewal_date < today',
    },
    {
      name: 'three-level nesting',
      dsl: 'status = "Potential" and (custom.industry = "saas" or custom.industry = "fintech") and not dnc = true',
    },
    {
      name: 'not over or-group with membership',
      dsl: 'not (status in ("Won", "Lost") or dnc = true)',
    },
    {
      name: 'or of empties (empty edge)',
      dsl: 'name = "ZZZ No Such Company" or status = "Archived"',
      expectEmpty: true,
    },
